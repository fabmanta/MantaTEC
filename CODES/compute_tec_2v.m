function [iec] = compute_tec_2v(L1,L2,C1,P1,P2,sv,tgd,ifb,emf_mat,type_sat,k)
...
if type_sat =='R' %GLONASS
    f1   =  1.602e9 + 0.5625.*k; %(Hz)
    f2   =  1.246e9 + 0.4375.*k; %(Hz)
elseif type_sat =='G'
    f1 = 1.57542e9;
    f2 = 1.2276e9;
end
...
%         iec(I(j):I(j+1)-1,i) = F * (+f2*tgd(i) +B+LG(I(j):I(j+1)-1,i) +f2*ifb);
        ...
        elseif type_sat =='R'
            iec(I(j):I(j+1)-1,i) = F(i) .* (+f2(i).*tmp_tgd(i) +B + LG(I(j):I(j+1)-1,i));
        end
...
elseif type_sat =='R'
    for i = 1:nsat
        iec(find(iec(i)),i) = iec(find(iec(i)),i)+ (F(i).*f2(i).*ifb_mean);
    end
end
